...
st.set_page_config(
    page_title="Heart Disease Insights Analysis",
    page_icon="👨‍⚕️"
)
...
def show_insight():
    st.title('👨‍⚕️ Heart Disease Insights Analysis')
    ...
    with st.expander("📋 Analisis Insight", expanded=True):
        st.info('Tampak bahwa umur 51 tahun ke atas cukup rawan terkena penyakit jantung. Semakin bertambahnya umur, manusia harus semakin menjaga pola hidupnya karena peluang terkena penyakit jantung semakin besar.')
        st.info('Hal tersebut masuk akal karena semakin bertambahnya usia, maka pembuluh darah cenderung mengalami penumpukan plak aterosklerotik (plak lemak) yang dapat menyebabkan penyempitan atau penyumbatan pembuluh darah koroner yang memasok darah ke jantung. Hal ini juga dapat meningkatkan risiko terjadinya penyakit jantung.')
    ...
    st.pyplot(plt)
    with st.expander("📋 Analisis Insight", expanded=True):
        st.info('Terlihat jelas bahwa sebagian besar orang yang menderita penyakit jantung memiliki maximum heart rate di bawah 130.')
        st.info('Jantung memiliki sistem listrik internal yang mengatur ritme dan frekuensi detak jantung. Gangguan pada sistem ini dapat menyebabkan detak jantung yang terlalu lambat.')
        st.info('Hal tersebut menjadi alasan mengapa kebanyakan penderita penyakit jantung memiliki detak jantung yang cukup rendah.')
    ...
    st.pyplot(plt)
    with st.expander("📋 Analisis Insight", expanded=True):
        st.info('Laki-laki lebih rawan menderita penyakit jantung. Tampak ada perbedaan yang sangat signifikan mengenai penderita penyakit jantung berdasarkan jenis kelaminnya.')
        st.info('Alasannya karena perempuan memiliki hormon estrogen yang diyakini memiliki efek pelindung dari penyakit jantung.')
    ...
    st.pyplot(plt)
    with st.expander("📋 Analisis Insight", expanded=True):
        st.info('Keterangan: ATA = Atypical Angina, NAP = Non-anginal Pain, AS = Asymptomatic, TA = Typical Angina')
        st.info('Chestpain asymptomatic adalah jenis sakit dada yang paling mengindikasikan adanya penyakit jantung. Tampak perbedaan yang sangat signifikan dengan jenis sakit dada yang lain.')
        st.info('Hal tersebut karena chestpain asymptomatic terjadi tanpa gejala sehinnga seringkali diabaikan oleh penderitanya. Ketika diabaikan, maka dapat semakin parah dan berujung pada penyakit jantung.')
    ...
    st.pyplot(plt)
    with st.expander("📋 Analisis Insight", expanded=True):
        st.info('Tampak bahwa orang dengan tingkat kolesterol di atas 251 mg/dL rawan menderita penyakit jantung.')
        st.info('Alasan: Kolestrol yang tinggi akan menyebabkan penumpukan plak lemak di dinding arteri. Akibatnya plak tersebut akan menyempitkan arteri dan mengurangi aliran darah yang kaya oksigen ke jantung. Hal inilah yang menyebabkan terjadinya penyakit jantung, terutama penyakit jantung koroner.')
    ...
    st.pyplot(plt)
    # st.info('Keterangan:')
    # st.info('0 = Kadar gula darah < 120 mg/dL')
    # st.info('1 = Kadar gula darah > 120 mg/dL')
    with st.expander("📋 Analisis Insight", expanded=True):
        st.info('Ya. Tampak bahwa sebagian besar orang yang memiliki kadar gula darah di atas 120 mg/dL memiliki penyakit jantung.')
        st.info('Hal tersebut masuk akal karena tingginya kadar gula darah dapat merusak pembuluh darah dan menyebabkan komplikasi penyakit jantung serius. Selain itu, gula darah yang tinggi dapat menyebabkan gangguan pada sistem kardiovaskular, seperti disfungsi otot jantung, aritmia (ketidakaturan detak jantung), dan kerusakan katup jantung.')
    ...
    st.pyplot(plt)
    with st.expander("📋 Analisis Insight", expanded=True):
        st.info('Tampak bahwa orang yang memiliki hasil tes depresi segmen ST di atas 1 rawan menderita penyakit jantung.')
        st.info('Dengan kata lain, semakin besar tingkat depresi segmen ST, maka semakin besar pula kemungkinan menderita penyakit jantung.')
        st.info('Hal tersebut terjadi karena depresi segmen ST sering kali menunjukkan adanya iskemia miokard, yaitu kondisi di mana aliran darah ke jantung berkurang. Penyebabnya adalah penyempitan atau penyumbatan arteri koroner yang memasok darah ke jantung.')
    ...
    st.pyplot(plt)
    with st.expander("📋 Analisis Insight", expanded=True):
        st.info('Ya, munculnya angina setelah berolahraga dapat menjadi indikator bahwa seseorang menderita penyakit jantung.')
        st.info('Tampak perbedaan yang sangat signifikan pada grafik di atas bahwa sebagian besar orang yang mengalami angina setelah olahraga ternyata menderita penyakit jantung.')
        st.info('Olahraga meningkatkan kebutuhan oksigen oleh otot jantung. Pada seseorang dengan penyakit arteri koroner atau penyakit jantung, pasokan darah dan oksigen ke otot jantung mungkin tidak mencukupi untuk memenuhi kebutuhan selama olahraga yang intens. Hal ini dapat menyebabkan iskemia miokard (kurangnya pasokan darah dan oksigen ke otot jantung), yang mana gejalanya sering dirasakan sebagai nyeri dada atau angina.')
        st.info('Oleh karena itu, jika seseorang mengalami angina setelah berolahraga, ini dapat menjadi tanda bahwa ada masalah dengan pasokan darah ke otot jantung dan dapat mengindikasikan adanya penyakit jantung. ')
    ...
    st.pyplot(plt)
    with st.expander("📋 Analisis Insight", expanded=True):
        st.info('Ya, kemiringan segmen ST dapat menjadi indikasi apakah seseorang menderita penyakit jantung atau tidak.')
        st.info('Jika hasil tes ST seseorang menunjukkan bahwa segmen ST-nya datar atau menurun, maka orang tersebut diindikasikan menderita penyakit jantung.')
        st.info('Hal tersebut karena segmen ST yang datar atau menurun dapat menjadi tanda bahwa terjadi iskemia miokard, yang terjadi ketika pasokan darah dan oksigen ke otot jantung terganggu. Segmen ST yang datar atau menurun juga dapat terjadi akibat peradangan atau inflamasi pada jantung, seperti pada penyakit pericarditis atau miokarditis. Kondisi ini juga dapat terkait dengan penyakit jantung tertentu.')
